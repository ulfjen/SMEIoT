...
export interface SensorsApiApiSensorsDeviceNameSensorNameLastGetRequest {
    deviceName: string;
    sensorName: string;
    seconds?: number;
}
...
export class SensorsApi extends runtime.BaseAPI {
    ...
    /**
     */
    async apiSensorsDeviceNameSensorNameLastGetRaw(requestParameters: SensorsApiApiSensorsDeviceNameSensorNameLastGetRequest): Promise<runtime.ApiResponse<SensorDetailsApiModel>> {
        if (requestParameters.deviceName === null || requestParameters.deviceName === undefined) {
            throw new runtime.RequiredError('deviceName','Required parameter requestParameters.deviceName was null or undefined when calling apiSensorsDeviceNameSensorNameLastGet.');
        }

        if (requestParameters.sensorName === null || requestParameters.sensorName === undefined) {
            throw new runtime.RequiredError('sensorName','Required parameter requestParameters.sensorName was null or undefined when calling apiSensorsDeviceNameSensorNameLastGet.');
        }
        ...
        const response = await this.request({
            path: `/api/sensors/{deviceName}/{sensorName}/last`.replace(`{${"deviceName"}}`, encodeURIComponent(String(requestParameters.deviceName))).replace(`{${"sensorName"}}`, encodeURIComponent(String(requestParameters.sensorName))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        });
    ...
    /**
     */
    async apiSensorsDeviceNameSensorNameLastGet(requestParameters: SensorsApiApiSensorsDeviceNameSensorNameLastGetRequest): Promise<SensorDetailsApiModel> {
        const response = await this.apiSensorsDeviceNameSensorNameLastGetRaw(requestParameters);
        return await response.value();
    }
